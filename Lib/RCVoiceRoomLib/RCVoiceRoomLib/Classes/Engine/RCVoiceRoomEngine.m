...
- (void)sendPKInvitation:(NSString *)inviteeRoomId
                          invitee:(NSString *)inviteeUserId
                           success:(RCVoiceRoomSuccessBlock)successBlock
                             error:(RCVoiceRoomErrorBlock)errorBlock {
    [self.rtcRoom.localUser requestJoinOtherRoom:inviteeRoomId userId:inviteeUserId autoMix:true extra:@"" completion:^(BOOL isSuccess, RCRTCCode code) {
        if (isSuccess) {
            successBlock();
        } else {
            NSLog([RCRTCCodeDefine codeDesc:code]);
...
- (void)cancelPKInvitation:(NSString *)inviteeRoomId
                      invitee:(NSString *)inviteeUserId
                       success:(RCVoiceRoomSuccessBlock)successBlock
                         error:(RCVoiceRoomErrorBlock)errorBlock {
    [self.rtcRoom.localUser cancelRequestJoinOtherRoom:inviteeRoomId userId:inviteeUserId extra:@"" completion:^(BOOL isSuccess, RCRTCCode code) {
        if (isSuccess) {
            successBlock();
        } else {
...
- (void)responsePKInvitation:(NSString *)inviterRoomId
                        ...
                        success:(RCVoiceRoomSuccessBlock)successBlock
                          error:(RCVoiceRoomErrorBlock)errorBlock {
    __weak typeof(self) weakSelf = self;
    if (type == RCPKResponseIgnore) {
        RCVoiceRoomRefreshMessage *ignoreMessage = [[RCVoiceRoomRefreshMessage alloc] init];
        ignoreMessage.name = RCIgnorePKInviteKey;
        ignoreMessage.content = [@[self.roomId, self.currentUserId] componentsJoinedByString:@","];
        [[RCCoreClient sharedCoreClient] sendMessage:ConversationType_PRIVATE targetId:inviterUserId content:ignoreMessage pushContent:nil pushData:nil success:^(long messageId) {
            successBlock();
        } error:^(RCErrorCode nErrorCode, long messageId) {
            errorBlock(RCVoiceRoomBeginPKFailed, @"response to join pk failed");
        }];
    } else {
        BOOL isAgree = (type == RCPKResponseAgree ? YES : NO);
        [self.rtcRoom.localUser responseJoinOtherRoom:inviterRoomId userId:inviterUserId agree:isAgree autoMix:YES extra:@"" completion:^(BOOL isSuccess, RCRTCCode code) {
            [self runMainQueue:^{
                if (isSuccess) {
                    if (isAgree) {
                        [weakSelf beginPKWithInviter:inviterUserId inviterRoom:inviterRoomId invitee:self.currentUserId inviteeRoom:self.roomId success:^{
                            successBlock();
                        } error:^(RCVoiceRoomErrorCode code, NSString * _Nonnull msg) {
        ...
        [[RCRTCEngine sharedInstance] leaveOtherRoom:otherRoomId notifyFinished:YES completion:^(BOOL isSuccess, RCRTCCode code) {
            [weakSelf runMainQueue:^{
                if (isSuccess) {
                    ...
                    NSLog(@"leave other room failed %d", code);
...
- (void)mutePKUser:(BOOL)isMute
           success:(RCVoiceRoomSuccessBlock)successBlock
             error:(RCVoiceRoomErrorBlock)errorBlock {
    __weak typeof(self) weakSelf = self;
    if (self.roomId == nil || self.currentPKInfo == nil) {
        errorBlock(RCVoiceRoomMutePKUserFailed, @"room id nil or pk not begin");
        return;
    }
    RCRTCMixConfig *streamConfig = [[RCRTCMixConfig alloc] init];
    streamConfig.customMode = YES;
    NSArray *streams = [self pkStreams];
    if (isMute) {
        [streamConfig setMixInputFilterByRoomIds:@[self.roomId] mixMediaType:RCRTCMixMediaType_Audio isAppend:NO];
        [self.rtcRoom.localUser unsubscribeStreams:streams completion:^(BOOL isSuccess, RCRTCCode code) {
            
        }];
    } else {
        NSString *otherRoomdId = ([self.roomId isEqualToString:self.currentPKInfo.inviterRoomId] ? self.currentPKInfo.inviteeRoomId : self.currentPKInfo.inviterRoomId);
        [streamConfig setMixInputFilterByRoomIds:@[self.roomId, otherRoomdId] mixMediaType:RCRTCMixMediaType_Audio isAppend:NO];
        if (streams.count > 0) {
    ...
    [self.liveInfo setMixConfig:streamConfig completion:^(BOOL isSuccess, RCRTCCode code) {
          [weakSelf runMainQueue:^{
              if (isSuccess) {
                  NSLog(@"set live info success");
                  successBlock();
              } else {
                  errorBlock(RCVoiceRoomMutePKUserFailed, @"Mute pk user failed");
              }
          }];
      }];
}
...
- (void)didReportStatusForm:(RCRTCStatusForm *)form {
    ...
    if ([self seatIndexInRange:userSeatIndex]) {
        RCVoiceSeatInfo *seatInfo = self.seatInfoList[userSeatIndex];
        if (seatInfo.isMuted) {
            return;
        }
...
- (void)didRequestJoinOtherRoom:(NSString *)inviterRoomId inviterUserId:(NSString *)inviterUserId extra:(NSString *)extra {
    __weak typeof(self) weakSelf = self;
    [self runMainQueue:^{
        if ([weakSelf.delegate respondsToSelector:@selector(pkInvitationDidReceiveFromRoom:byUser:)]) {
            [weakSelf.delegate pkInvitationDidReceiveFromRoom:inviterRoomId byUser:inviterUserId];
        }
    }];
}
...
- (void)didCancelRequestOtherRoom:(NSString *)inviterRoomId inviterUserId:(NSString *)inviterUserId extra:(NSString *)extra {
    __weak typeof(self) weakSelf = self;
    [self runMainQueue:^{
        if ([weakSelf.delegate respondsToSelector:@selector(cancelPKInvitationDidReceiveFromRoom:byUser:)]) {
            [weakSelf.delegate cancelPKInvitationDidReceiveFromRoom:inviterRoomId byUser:inviterUserId];
        }
    }];
}
...
- (void)didResponseJoinOtherRoom:(NSString *)inviterRoomId inviterUserId:(NSString *)inviterUserId inviteeRoomId:(NSString *)inviteeRoomId inviteeUserId:(NSString *)inviteeUserId agree:(BOOL)agree extra:(NSString *)extra {
    ...
    if ([self.currentUserId isEqualToString:inviterUserId]) {
        ...
        if (agree) {
            ...
            [self runMainQueue:^{
                if ([weakSelf.delegate respondsToSelector:@selector(rejectPKInvitationDidReceiveFromRoom:byUser:)]) {
                    [weakSelf.delegate rejectPKInvitationDidReceiveFromRoom:inviteeRoomId byUser:inviteeUserId];
                }
            }];
        }
    }
...
- (void)onReceived:(RCMessage *)message left:(int)nLeft object:(id)object {
    ...
    if ([self ifCouldTransfer:message]) {
        ...
        [self runMainQueue:^{
            ...
            [weakSelf handlePKMessage:message];
        }];
    }
}
...
- (NSArray<RCRTCInputStream *> *)pkStreams {
    NSMutableArray *streams = [NSMutableArray array];
    for (RCRTCRemoteUser *remoteUser in self.otherRoom.remoteUsers) {
...
- (void)syncPKInfo:(NSString *)content {
...
- (void)handleRefreshMessage:(RCMessage *)message {
    if ([message.content isKindOfClass:[RCVoiceRoomRefreshMessage class]]) {
        ...
        if (message.conversationType == ConversationType_PRIVATE) {
            if ([refreshMessage.name isEqualToString:RCIgnorePKInviteKey]) {
                NSArray *inviteeInfo = [refreshMessage.content componentsSeparatedByString:@","];
                if (inviteeInfo.count == 2) {
                    NSString *inviteeRoomId = inviteeInfo[0];
                    NSString *inviteeUserId = inviteeInfo[1];
                    [self cancelPKInvitation:inviteeRoomId invitee:inviteeUserId success:^{
                        
                    } error:^(RCVoiceRoomErrorCode code, NSString * _Nonnull msg) {
                        
                    }];
                    if ([self.delegate respondsToSelector:@selector(ignorePKInvitationDidReceiveFromRoom:byUser:)]) {
                        [self.delegate ignorePKInvitationDidReceiveFromRoom:inviteeRoomId byUser:inviteeUserId];
                    }
                }
            }
        }
...
- (void)handlePKMessage:(RCMessage *)message {
    ...
    [[RCRTCEngine sharedInstance] joinRoom:roomId config:config completion:^(RCRTCRoom * _Nullable room, RCRTCCode code) {
        if (code == RCRTCCodeSuccess) {
            ...
            if (role == RCRTCLiveRoleTypeBroadcaster) {
                [weakSelf.rtcRoom.localUser publishDefaultLiveStreams:^(BOOL isSuccess, RCRTCCode code, RCRTCLiveInfo * _Nullable liveInfo) {
                    weakSelf.liveInfo = liveInfo;
                }];
                NSMutableArray *streams = [NSMutableArray array];
